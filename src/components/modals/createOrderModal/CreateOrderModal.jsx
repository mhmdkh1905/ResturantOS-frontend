...
export default function CreateOrderModal({ isOpen, onClose, onCreate }) {
  ...
  const addItem = useCallback(() => {
    if (selectedMenuItem && currentQty > 0) {
      setOrderItems((prev) => [
        ...prev,
        { ...selectedMenuItem, qty: currentQty },
      ]);
      setMenuSearch("");
        ...
        <form onSubmit={handleSubmit} className={styles.form}>
          ...
          <div className={styles.field}>
            ...
            <div className={styles.selectWrapper}>
              ...
              >
                ...
                {tables.map((table) => {
                  const safeStatus =
                    table.status?.name || table.status || "free";
                  return (
                    <option key={table.id} value={table.id}>
                      Table #{table.number ?? "—"}{" "}
                      {safeStatus === "occupied" ? "(Occupied)" : "(Free)"}
                    </option>
                  );
                })}
              </select>
          ...
          {orderItems.length > 0 && (
            <div className={styles.itemsList}>
              <h4>
                Order Items ({orderItems.reduce((sum, i) => sum + i.qty, 0)}{" "}
                items)
              </h4>
                ...
                <div key={index} className={styles.orderItemRow}>
                  ...
                  <div className={styles.orderItemControls}>
                    <div className={styles.stepperSmall}>
                      <button
                        type="button"
                        onClick={() => updateOrderItemQty(index, -1)}
                        className={styles.stepBtnSmall}
                      >
                        <Minus size={14} />
                      </button>
                      <span>{item.qty}</span>
          ...
          <div className={styles.actions}>
            ...
            <button
              type="submit"
              disabled={
                !selectedTableId || orderItems.length === 0 || isSubmitting
              }
